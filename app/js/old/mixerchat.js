...
//module
let beamchat = function(authToken, chatConnected) {
    ...
    if (authToken) {
        ...
        console.log('using streamer access access' + authToken);
        ...
        function createChatSocket(userId, channelId, endpoints, authkey, chatConnected) {
            ...
            console.log('user connectiing to chat is: ' + userId);
            ...
            socket.on('error', error => {
                ...
                console.log('socket error, Socket info is: ' + error);
                ...
                if (!chatConnected) {
                    console.log(' Reconnecting bot to chat...');
...
function connectToBeam(client, authToken, createChatSocket, self, useAuth, chatConnected) {
    ...
    //this is used when reconnecting
    if (useAuth) {
        client.use(new Mixer.OAuthProvider(client, {
            tokens: {
                access: authToken,
    ...
    client.request('GET', `users/current`)
        .then(response => {
            ...
            console.log('Getting Current User data response  user id: ' + response.body.id + ' username:' + response.body.username);
        ...
        .then(response => {
            ...
            console.log('streamer id ' + userInfo.id + ' is joining channel ' + beamChannelID);
