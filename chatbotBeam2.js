    ...
    */
    ...
    socket.on('alertFinished', function(dateid, type) {
        try {
            ...
            GetFirstOneAndDeletesIt(dateid, type);
        } catch (error) {
    ...
    socket.on('addSaveTrigger', function(cmdObject) {
        try {
            ...
            var commandExists = checkTriggerUIExists(cmdObject);

            if (commandExists.length == 0) {

                processAddUITrigger(cmdObject);
                io.emit('addSaveSingleTrigger', cmdObject);
            } else {
                processEditUITrigger(cmdObject);
                io.emit('addSaveSingleTrigger', cmdObject);
            }
    ...
    socket.on('addSaveNote', function(cmdObject) {
        try {
            ...
            var commandExists = checkNoteUIExists(cmdObject);
    ...
    socket.on('deleteTrigger', function(cmdObject) {
        try {
            ...
            if (commandExists.length > 0) {
                ...
                deleteTriggerFromList(cmdObject);
    ...
    socket.on('completeNote', function(cmdObject) {
        try {
            ...
            if (commandExists.length > 0) {
                ...
                completeNoteFromList(cmdObject);
                io.emit('addSaveSingleNote', cmdObject);
    ...
    socket.on('addSaveTimer', function(cmdObject) {
        try {
            ...
            var commandExists = checkTimerUIExists(cmdObject);
    ...
    socket.on('addSaveMedia', function(cmdMedia) {
        try {
            ...
            var mediaExists = checkMediaUIExists(cmdMedia);

            if (mediaExists.length == 0) {
                processAddUIMedia(cmdMedia);
            } else {
                processEditUIMedia(cmdMedia);
            }
...
function deleteTimerFromList(cmdObject) {
...
function deleteTriggerFromList(cmdObject) {
...
function deleteNoteFromList(cmdObject) {
...
function ConnectToBeamAndConsellation() {
    ...
    if (!isConnected) {
        ...
        CreateBeamBotObjects(passport.BBBTokenBot, null, passport.username, chatConnectedBot, passport.username, passport.usernameBot, passport.streamerChannelID);
...
function SendMediaListToBot(myMedia) {
    ...
    audioMedia = myMedia.data.media.filter(function(item) { return (item.type == 'Audio'); });
    imageMedia = myMedia.data.media.filter(function(item) { return (item.type == 'Image'); });
    videoMedia = myMedia.data.media.filter(function(item) { return (item.type == 'Video'); });
        ...
        if (bcBot != null) {
            ...
            bcBot.on('ChatMessage', function(data) {
                ...
                if (text[0].substr(0, 1) == "!") {
                    ...
                    //send to commandHandler to determine what to send to mixer
                    processChatCommand(userCommands, text[0], data.user_roles, UserName, ch, channelToken, t, bc);
...
function processChatCommand(CommandList, command, user_roles, username, ch, streamerToken, fullcommand, bc) {
    try {
...
function processAddUITrigger(fullcommand) {
...
function processAddUINote(fullcommand) {
...
function processEditUIAlert(alertObject) {
    //need to work on feedback what happens if its successfull / unsuccessfull
...
//this edits the command
function processEditUICommand(fullcommand) {
    //need to work on feedback what happens if its successfull / unsuccessfull
...
function processEditUITrigger(fullcommand) {
    //need to work on feedback what happens if its successfull / unsuccessfull
...
function completeNoteFromList(fullcommand) {
    //need to work on feedback what happens if its successfull / unsuccessfull
...
function processEditUIMedia(mediaObject) {
    //need to work on feedback what happens if its successfull / unsuccessfull
...
function checkCommandUIExists(fullcommand) {
...
function checkTriggerUIExists(fullcommand) {
...
function checkNoteUIExists(fullcommand) {
...
function checkTimerUIExists(fullcommand) {
...
function checkMediaUIExists(mediaObject) {
...
function GetFirstOneAndDeletesIt(dateidIn, typeIn) {
...
function GetLastFromList(message) {
    //item in json deletion
    //  dbAlerts.delete("/alerts" + userName);
    //array item deletion
...
function initBeamData() {
    ...
    if (bc != null) {
        ...
        bc.getStreamerFollows(passport.userID);

        bc.getfollowers(passport.streamerChannelID);

        bc.getChatUsers(passport.streamerChannelID);
...
function addAlertToDB(alertMSG) {
...
function addAlltimersSchedule(myTimers) {
    ...
    for (var timerCnt = 0, len = myTimers.data.timers.length; timerCnt < len; timerCnt++) {
        ...
        Scheduler.add(function() { SendMessageToBeam(textForTimer) }, null, duration2 / 10, true);
...
var example2 = function() {
    ...
    for (var timerCnt = 0, len = myTimers.data.timers.length; timerCnt < len; timerCnt++) {
        ...
        var timerObject = myTimers.data.timers[timerCnt];
        var message2 = myTimers.data.timers[timerCnt].text;
        var duration2 = myTimers.data.timers[timerCnt].interval;
        // = new Object(); 
        let testObj = myTimers.data.timers[timerCnt].text.toString();


        Scheduler.add(function() { SendMessageToBeam(testObj) }, null, duration2 / 100, false);
